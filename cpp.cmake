...
set(_JLN_DIAGNOSTICS_SHOW_TEMPLATE_VALUES default tree without_elided_types tree_without_elided_types)
...
set(_JLN_MSVC_ISYSTEM_WITH_TEMPLATE_INSTANTIATIONS_TREATED_AS_NON_EXTERNAL_VALUES default off on)
...
set(_JLN_OPTIMIZATION_WARNINGS_VALUES default off on)
...
set(_JLN_SANITIZERS_VALUES default off on extra address kernel kernel_extra kernel_extra kernel_address thread undefined undefined_minimal_runtime scudo_hardened_allocator)
...
set(_JLN_STL_HARDENING_VALUES default off fast extensive debug debug_with_broken_abi)
...
set(_JLN_WINDOWS_ABI_COMPATIBILITY_WARNINGS_VALUES default off on)
...
set(JLN_DIAGNOSTICS_SHOW_TEMPLATE "${JLN_DIAGNOSTICS_SHOW_TEMPLATE}" CACHE STRING "Configures how templates with incompatible types are displayed (Clang and GCC only).\n - tree: Enables printing a tree-like structure showing the common and differing parts of the types\n - without_elided_types: Disables printing diagnostics showing common parts of template types as \"[...]\".\n - tree_without_elided_types: Prints a tree-like without replacing common types. Mergeing tree and without_elided_types.")
set_property(CACHE JLN_DIAGNOSTICS_SHOW_TEMPLATE PROPERTY STRINGS "default" "tree" "without_elided_types" "tree_without_elided_types")
if(NOT("${JLN_DIAGNOSTICS_SHOW_TEMPLATE}" STREQUAL ""))
  string(TOLOWER "${JLN_DIAGNOSTICS_SHOW_TEMPLATE}" JLN_DIAGNOSTICS_SHOW_TEMPLATE)
  if(NOT(("default" STREQUAL JLN_DIAGNOSTICS_SHOW_TEMPLATE) OR ("tree" STREQUAL JLN_DIAGNOSTICS_SHOW_TEMPLATE) OR ("without_elided_types" STREQUAL JLN_DIAGNOSTICS_SHOW_TEMPLATE) OR ("tree_without_elided_types" STREQUAL JLN_DIAGNOSTICS_SHOW_TEMPLATE)))
    message(FATAL_ERROR "Unknow value \"${JLN_DIAGNOSTICS_SHOW_TEMPLATE}\" for JLN_DIAGNOSTICS_SHOW_TEMPLATE, expected: default, tree, without_elided_types, tree_without_elided_types")
  endif()
endif()
...
set(JLN_MSVC_ISYSTEM_WITH_TEMPLATE_INSTANTIATIONS_TREATED_AS_NON_EXTERNAL "${JLN_MSVC_ISYSTEM_WITH_TEMPLATE_INSTANTIATIONS_TREATED_AS_NON_EXTERNAL}" CACHE STRING "Allows warnings from external headers when they occur in a template that's instantiated in your code. (requires msvc_isystem).")
set_property(CACHE JLN_MSVC_ISYSTEM_WITH_TEMPLATE_INSTANTIATIONS_TREATED_AS_NON_EXTERNAL PROPERTY STRINGS "default" "off" "on")
if(NOT("${JLN_MSVC_ISYSTEM_WITH_TEMPLATE_INSTANTIATIONS_TREATED_AS_NON_EXTERNAL}" STREQUAL ""))
  string(TOLOWER "${JLN_MSVC_ISYSTEM_WITH_TEMPLATE_INSTANTIATIONS_TREATED_AS_NON_EXTERNAL}" JLN_MSVC_ISYSTEM_WITH_TEMPLATE_INSTANTIATIONS_TREATED_AS_NON_EXTERNAL)
  if(NOT(("default" STREQUAL JLN_MSVC_ISYSTEM_WITH_TEMPLATE_INSTANTIATIONS_TREATED_AS_NON_EXTERNAL) OR ("off" STREQUAL JLN_MSVC_ISYSTEM_WITH_TEMPLATE_INSTANTIATIONS_TREATED_AS_NON_EXTERNAL) OR ("on" STREQUAL JLN_MSVC_ISYSTEM_WITH_TEMPLATE_INSTANTIATIONS_TREATED_AS_NON_EXTERNAL)))
    message(FATAL_ERROR "Unknow value \"${JLN_MSVC_ISYSTEM_WITH_TEMPLATE_INSTANTIATIONS_TREATED_AS_NON_EXTERNAL}\" for JLN_MSVC_ISYSTEM_WITH_TEMPLATE_INSTANTIATIONS_TREATED_AS_NON_EXTERNAL, expected: default, off, on")
  endif()
endif()
...
set(JLN_NOEXCEPT_WARNINGS "${JLN_NOEXCEPT_WARNINGS}" CACHE STRING "Warn when a noexcept-expression evaluates to false because of a call to a function that does not have a non-throwing exception specification (i.e. \"throw()\" or \"noexcept\") but is known by the compiler to never throw an exception. Only with GCC.")
...
set(JLN_OPTIMIZATION_WARNINGS "${JLN_OPTIMIZATION_WARNINGS}" CACHE STRING "Activate warnings about optimization.")
set_property(CACHE JLN_OPTIMIZATION_WARNINGS PROPERTY STRINGS "default" "off" "on")
if(NOT("${JLN_OPTIMIZATION_WARNINGS}" STREQUAL ""))
  string(TOLOWER "${JLN_OPTIMIZATION_WARNINGS}" JLN_OPTIMIZATION_WARNINGS)
  if(NOT(("default" STREQUAL JLN_OPTIMIZATION_WARNINGS) OR ("off" STREQUAL JLN_OPTIMIZATION_WARNINGS) OR ("on" STREQUAL JLN_OPTIMIZATION_WARNINGS)))
    message(FATAL_ERROR "Unknow value \"${JLN_OPTIMIZATION_WARNINGS}\" for JLN_OPTIMIZATION_WARNINGS, expected: default, off, on")
  endif()
endif()
...
set(JLN_RELRO "${JLN_RELRO}" CACHE STRING "Specifies a memory segment that should be made read-only after relocation, if supported.")
...
set(JLN_RTTI "${JLN_RTTI}" CACHE STRING "Disable generation of information about every class with virtual functions for use by the C++ run-time type identification features (\"dynamic_cast\" and \"typeid\").")
...
set_property(CACHE JLN_SANITIZERS PROPERTY STRINGS "default" "off" "on" "extra" "address" "kernel" "kernel_extra" "kernel_extra" "kernel_address" "thread" "undefined" "undefined_minimal_runtime" "scudo_hardened_allocator")
if(NOT("${JLN_SANITIZERS}" STREQUAL ""))
  string(TOLOWER "${JLN_SANITIZERS}" JLN_SANITIZERS)
  if(NOT(("default" STREQUAL JLN_SANITIZERS) OR ("off" STREQUAL JLN_SANITIZERS) OR ("on" STREQUAL JLN_SANITIZERS) OR ("extra" STREQUAL JLN_SANITIZERS) OR ("address" STREQUAL JLN_SANITIZERS) OR ("kernel" STREQUAL JLN_SANITIZERS) OR ("kernel_extra" STREQUAL JLN_SANITIZERS) OR ("kernel_extra" STREQUAL JLN_SANITIZERS) OR ("kernel_address" STREQUAL JLN_SANITIZERS) OR ("thread" STREQUAL JLN_SANITIZERS) OR ("undefined" STREQUAL JLN_SANITIZERS) OR ("undefined_minimal_runtime" STREQUAL JLN_SANITIZERS) OR ("scudo_hardened_allocator" STREQUAL JLN_SANITIZERS)))
    message(FATAL_ERROR "Unknow value \"${JLN_SANITIZERS}\" for JLN_SANITIZERS, expected: default, off, on, extra, address, kernel, kernel_extra, kernel_extra, kernel_address, thread, undefined, undefined_minimal_runtime, scudo_hardened_allocator")
  endif()
endif()
...
set(JLN_STL_HARDENING "${JLN_STL_HARDENING}" CACHE STRING "Hardening allows turning some instances of undefined behavior in the standard library into a contract violation.\n - fast: A set of security-critical checks that can be done with relatively little overhead in constant time and are intended to be used in production. No impact on the ABI.\n - extensive: All the checks from fast mode and some additional checks for undefined behavior that incur relatively little overhead but aren’t security-critical. No impact on the ABI.\n - debug: Enables all the available checks, including heuristic checks that might have significant performance overhead as well as internal library assertions. No impact on the ABI.\n - debug_with_broken_abi: Debug mode with ABI incompatibility for more check.")
set_property(CACHE JLN_STL_HARDENING PROPERTY STRINGS "default" "off" "fast" "extensive" "debug" "debug_with_broken_abi")
if(NOT("${JLN_STL_HARDENING}" STREQUAL ""))
  string(TOLOWER "${JLN_STL_HARDENING}" JLN_STL_HARDENING)
  if(NOT(("default" STREQUAL JLN_STL_HARDENING) OR ("off" STREQUAL JLN_STL_HARDENING) OR ("fast" STREQUAL JLN_STL_HARDENING) OR ("extensive" STREQUAL JLN_STL_HARDENING) OR ("debug" STREQUAL JLN_STL_HARDENING) OR ("debug_with_broken_abi" STREQUAL JLN_STL_HARDENING)))
    message(FATAL_ERROR "Unknow value \"${JLN_STL_HARDENING}\" for JLN_STL_HARDENING, expected: default, off, fast, extensive, debug, debug_with_broken_abi")
  endif()
endif()
...
set(JLN_WINDOWS_ABI_COMPATIBILITY_WARNINGS "${JLN_WINDOWS_ABI_COMPATIBILITY_WARNINGS}" CACHE STRING "In code that is intended to be portable to Windows-based compilers the warning helps prevent unresolved references due to the difference in the mangling of symbols declared with different class-keys.")
set_property(CACHE JLN_WINDOWS_ABI_COMPATIBILITY_WARNINGS PROPERTY STRINGS "default" "off" "on")
if(NOT("${JLN_WINDOWS_ABI_COMPATIBILITY_WARNINGS}" STREQUAL ""))
  string(TOLOWER "${JLN_WINDOWS_ABI_COMPATIBILITY_WARNINGS}" JLN_WINDOWS_ABI_COMPATIBILITY_WARNINGS)
  if(NOT(("default" STREQUAL JLN_WINDOWS_ABI_COMPATIBILITY_WARNINGS) OR ("off" STREQUAL JLN_WINDOWS_ABI_COMPATIBILITY_WARNINGS) OR ("on" STREQUAL JLN_WINDOWS_ABI_COMPATIBILITY_WARNINGS)))
    message(FATAL_ERROR "Unknow value \"${JLN_WINDOWS_ABI_COMPATIBILITY_WARNINGS}\" for JLN_WINDOWS_ABI_COMPATIBILITY_WARNINGS, expected: default, off, on")
  endif()
endif()
...
function(jln_init_flags)
  jln_parse_arguments(JLN_DEFAULT_FLAG "VERBOSE;ANALYZER;ANALYZER_TOO_COMPLEX_WARNING;ANALYZER_VERBOSITY;COLOR;CONTROL_FLOW;CONVERSION_WARNINGS;COVERAGE;COVERED_SWITCH_DEFAULT_WARNINGS;CPU;DIAGNOSTICS_FORMAT;DIAGNOSTICS_SHOW_TEMPLATE;EXCEPTIONS;LINKER;LTO;MSVC_CRT_SECURE_NO_WARNINGS;MSVC_DIAGNOSTICS_FORMAT;MSVC_ISYSTEM;MSVC_ISYSTEM_WITH_TEMPLATE_INSTANTIATIONS_TREATED_AS_NON_EXTERNAL;NDEBUG;NOEXCEPT_WARNINGS;OPTIMIZATION;OPTIMIZATION_WARNINGS;PEDANTIC;PIE;RELRO;REPRODUCIBLE_BUILD_WARNINGS;RTTI;SANITIZERS;SHADOW_WARNINGS;STACK_PROTECTOR;STL_FIX;STL_HARDENING;SUGGEST_ATTRIBUTES;SWITCH_WARNINGS;SYMBOLS;UNSAFE_BUFFER_USAGE_WARNINGS;VAR_INIT;WARNINGS;WARNINGS_AS_ERROR;WINDOWS_ABI_COMPATIBILITY_WARNINGS;WINDOWS_BIGOBJ;AUTO_PROFILE" ${ARGN})
  ...
  if(DEFINED JLN_DEFAULT_FLAG_DIAGNOSTICS_SHOW_TEMPLATE)
    set(JLN_DIAGNOSTICS_SHOW_TEMPLATE_D ${JLN_DEFAULT_FLAG_DIAGNOSTICS_SHOW_TEMPLATE} CACHE STRING "private" FORCE)
  elseif("${JLN_DIAGNOSTICS_SHOW_TEMPLATE}" STREQUAL "")
    set(JLN_DIAGNOSTICS_SHOW_TEMPLATE_D "default" CACHE STRING "private" FORCE)
  else()
    set(JLN_DIAGNOSTICS_SHOW_TEMPLATE_D "${JLN_DIAGNOSTICS_SHOW_TEMPLATE}" CACHE STRING "private" FORCE)
  endif()
  ...
  if(DEFINED JLN_DEFAULT_FLAG_MSVC_ISYSTEM_WITH_TEMPLATE_INSTANTIATIONS_TREATED_AS_NON_EXTERNAL)
    set(JLN_MSVC_ISYSTEM_WITH_TEMPLATE_INSTANTIATIONS_TREATED_AS_NON_EXTERNAL_D ${JLN_DEFAULT_FLAG_MSVC_ISYSTEM_WITH_TEMPLATE_INSTANTIATIONS_TREATED_AS_NON_EXTERNAL} CACHE STRING "private" FORCE)
  elseif("${JLN_MSVC_ISYSTEM_WITH_TEMPLATE_INSTANTIATIONS_TREATED_AS_NON_EXTERNAL}" STREQUAL "")
    set(JLN_MSVC_ISYSTEM_WITH_TEMPLATE_INSTANTIATIONS_TREATED_AS_NON_EXTERNAL_D "default" CACHE STRING "private" FORCE)
  else()
    set(JLN_MSVC_ISYSTEM_WITH_TEMPLATE_INSTANTIATIONS_TREATED_AS_NON_EXTERNAL_D "${JLN_MSVC_ISYSTEM_WITH_TEMPLATE_INSTANTIATIONS_TREATED_AS_NON_EXTERNAL}" CACHE STRING "private" FORCE)
  endif()
  ...
  if(DEFINED JLN_DEFAULT_FLAG_OPTIMIZATION_WARNINGS)
    set(JLN_OPTIMIZATION_WARNINGS_D ${JLN_DEFAULT_FLAG_OPTIMIZATION_WARNINGS} CACHE STRING "private" FORCE)
  elseif("${JLN_OPTIMIZATION_WARNINGS}" STREQUAL "")
    set(JLN_OPTIMIZATION_WARNINGS_D "default" CACHE STRING "private" FORCE)
  else()
    set(JLN_OPTIMIZATION_WARNINGS_D "${JLN_OPTIMIZATION_WARNINGS}" CACHE STRING "private" FORCE)
  endif()
  ...
  if(DEFINED JLN_DEFAULT_FLAG_STL_HARDENING)
    set(JLN_STL_HARDENING_D ${JLN_DEFAULT_FLAG_STL_HARDENING} CACHE STRING "private" FORCE)
  elseif("${JLN_STL_HARDENING}" STREQUAL "")
    set(JLN_STL_HARDENING_D "default" CACHE STRING "private" FORCE)
  else()
    set(JLN_STL_HARDENING_D "${JLN_STL_HARDENING}" CACHE STRING "private" FORCE)
  endif()
  ...
  if(DEFINED JLN_DEFAULT_FLAG_WINDOWS_ABI_COMPATIBILITY_WARNINGS)
    set(JLN_WINDOWS_ABI_COMPATIBILITY_WARNINGS_D ${JLN_DEFAULT_FLAG_WINDOWS_ABI_COMPATIBILITY_WARNINGS} CACHE STRING "private" FORCE)
  elseif("${JLN_WINDOWS_ABI_COMPATIBILITY_WARNINGS}" STREQUAL "")
    set(JLN_WINDOWS_ABI_COMPATIBILITY_WARNINGS_D "off" CACHE STRING "private" FORCE)
  else()
    set(JLN_WINDOWS_ABI_COMPATIBILITY_WARNINGS_D "${JLN_WINDOWS_ABI_COMPATIBILITY_WARNINGS}" CACHE STRING "private" FORCE)
  endif()
  ...
  if("${JLN_VERBOSE_D}" STREQUAL "on" OR "${JLN_VERBOSE_D}" STREQUAL "1")
    ...
    message(STATUS "JLN_DIAGNOSTICS_SHOW_TEMPLATE = ${JLN_DIAGNOSTICS_SHOW_TEMPLATE_D}	[default, tree, without_elided_types, tree_without_elided_types]")
    ...
    message(STATUS "JLN_MSVC_ISYSTEM_WITH_TEMPLATE_INSTANTIATIONS_TREATED_AS_NON_EXTERNAL = ${JLN_MSVC_ISYSTEM_WITH_TEMPLATE_INSTANTIATIONS_TREATED_AS_NON_EXTERNAL_D}	[default, off, on]")
    ...
    message(STATUS "JLN_OPTIMIZATION_WARNINGS = ${JLN_OPTIMIZATION_WARNINGS_D}	[default, off, on]")
    ...
    message(STATUS "JLN_SANITIZERS = ${JLN_SANITIZERS_D}	[default, off, on, extra, address, kernel, kernel_extra, kernel_extra, kernel_address, thread, undefined, undefined_minimal_runtime, scudo_hardened_allocator]")
    ...
    message(STATUS "JLN_STL_HARDENING = ${JLN_STL_HARDENING_D}	[default, off, fast, extensive, debug, debug_with_broken_abi]")
    ...
    message(STATUS "JLN_WINDOWS_ABI_COMPATIBILITY_WARNINGS = ${JLN_WINDOWS_ABI_COMPATIBILITY_WARNINGS_D}	[default, off, on]")
...
set(JLN_CXX_COMPILER_VERSION ${CMAKE_CXX_COMPILER_VERSION})
...
elseif(CMAKE_CXX_COMPILER_ID MATCHES "Clang")
  if (CMAKE_CXX_COMPILER MATCHES /em\\+\\+)
    ...
    set(JLN_CLANG_EMCC_CXX_COMPILER 1)
  elseif(MSVC)
...
elseif(CMAKE_CXX_COMPILER_ID MATCHES "IntelLLVM")
  ...
  set(JLN_CXX_COMPILER_VERSION "${CMAKE_MATCH_1}.${CMAKE_MATCH_2}.${CMAKE_MATCH_3}")
# icc / icl
elseif(CMAKE_CXX_COMPILER_ID MATCHES "Intel")
  if (CMAKE_HOST_WIN32)
    set(JLN_ICL_CXX_COMPILER 1)
  else()
    set(JLN_ICC_CXX_COMPILER 1)
  endif()
endif()
...
function(jln_flags)
  ...
  set(CXX_FLAGS "")
  set(LINK_LINK "")
  jln_parse_arguments(JLN_FLAGS "DISABLE_OTHERS;CXX_VAR;LINK_VAR;ANALYZER;ANALYZER_TOO_COMPLEX_WARNING;ANALYZER_VERBOSITY;COLOR;CONTROL_FLOW;CONVERSION_WARNINGS;COVERAGE;COVERED_SWITCH_DEFAULT_WARNINGS;CPU;DIAGNOSTICS_FORMAT;DIAGNOSTICS_SHOW_TEMPLATE;EXCEPTIONS;LINKER;LTO;MSVC_CRT_SECURE_NO_WARNINGS;MSVC_DIAGNOSTICS_FORMAT;MSVC_ISYSTEM;MSVC_ISYSTEM_WITH_TEMPLATE_INSTANTIATIONS_TREATED_AS_NON_EXTERNAL;NDEBUG;NOEXCEPT_WARNINGS;OPTIMIZATION;OPTIMIZATION_WARNINGS;PEDANTIC;PIE;RELRO;REPRODUCIBLE_BUILD_WARNINGS;RTTI;SANITIZERS;SHADOW_WARNINGS;STACK_PROTECTOR;STL_FIX;STL_HARDENING;SUGGEST_ATTRIBUTES;SWITCH_WARNINGS;SYMBOLS;UNSAFE_BUFFER_USAGE_WARNINGS;VAR_INIT;WARNINGS;WARNINGS_AS_ERROR;WINDOWS_ABI_COMPATIBILITY_WARNINGS;WINDOWS_BIGOBJ" ${ARGN})
  ...
  if(NOT DEFINED JLN_FLAGS_DIAGNOSTICS_SHOW_TEMPLATE)
    if(JLN_FLAGS_DISABLE_OTHERS)
      set(JLN_FLAGS_DIAGNOSTICS_SHOW_TEMPLATE "default")
    else()
      set(JLN_FLAGS_DIAGNOSTICS_SHOW_TEMPLATE "${JLN_DIAGNOSTICS_SHOW_TEMPLATE_D}")
    endif()
  endif()
  ...
  if(NOT DEFINED JLN_FLAGS_MSVC_ISYSTEM_WITH_TEMPLATE_INSTANTIATIONS_TREATED_AS_NON_EXTERNAL)
    if(JLN_FLAGS_DISABLE_OTHERS)
      set(JLN_FLAGS_MSVC_ISYSTEM_WITH_TEMPLATE_INSTANTIATIONS_TREATED_AS_NON_EXTERNAL "default")
    else()
      set(JLN_FLAGS_MSVC_ISYSTEM_WITH_TEMPLATE_INSTANTIATIONS_TREATED_AS_NON_EXTERNAL "${JLN_MSVC_ISYSTEM_WITH_TEMPLATE_INSTANTIATIONS_TREATED_AS_NON_EXTERNAL_D}")
    endif()
  endif()
  ...
  if(NOT DEFINED JLN_FLAGS_OPTIMIZATION_WARNINGS)
    if(JLN_FLAGS_DISABLE_OTHERS)
      set(JLN_FLAGS_OPTIMIZATION_WARNINGS "default")
    else()
      set(JLN_FLAGS_OPTIMIZATION_WARNINGS "${JLN_OPTIMIZATION_WARNINGS_D}")
    endif()
  endif()
  ...
  if(NOT DEFINED JLN_FLAGS_STL_HARDENING)
    if(JLN_FLAGS_DISABLE_OTHERS)
      set(JLN_FLAGS_STL_HARDENING "default")
    else()
      set(JLN_FLAGS_STL_HARDENING "${JLN_STL_HARDENING_D}")
    endif()
  endif()
  ...
  if(NOT DEFINED JLN_FLAGS_WINDOWS_ABI_COMPATIBILITY_WARNINGS)
    if(JLN_FLAGS_DISABLE_OTHERS)
      set(JLN_FLAGS_WINDOWS_ABI_COMPATIBILITY_WARNINGS "off")
    else()
      set(JLN_FLAGS_WINDOWS_ABI_COMPATIBILITY_WARNINGS "${JLN_WINDOWS_ABI_COMPATIBILITY_WARNINGS_D}")
    endif()
  endif()
  ...
  if (   NOT ( JLN_FLAGS_NDEBUG STREQUAL "default" ) )
    if (  ( JLN_MSVC_CXX_COMPILER OR JLN_ICL_CXX_COMPILER ) )
  ...
  if (  ( JLN_GCC_CXX_COMPILER OR JLN_CLANG_LIKE_COMPILER OR JLN_CLANG_CL_CXX_COMPILER ) )
      ...
      if (  JLN_FLAGS_WARNINGS STREQUAL "off" )
        ...
        if (  JLN_FLAGS_WARNINGS STREQUAL "essential" )
          ...
          if (  JLN_GCC_CXX_COMPILER )
            list(APPEND CXX_FLAGS  "-Wall" "-Wextra")
            if (  JLN_CXX_COMPILER_VERSION VERSION_LESS "8.0" )
              list(APPEND CXX_FLAGS  "-Wcast-align")
            endif()
            list(APPEND CXX_FLAGS  "-Wcast-qual" "-Wdisabled-optimization" "-Wfloat-equal" "-Wformat-security" "-Wformat=2" "-Winvalid-pch" "-Wmissing-declarations" "-Wmissing-include-dirs" "-Wpacked" "-Wredundant-decls" "-Wundef" "-Wunused-macros" "-Wpointer-arith" "-Wnon-virtual-dtor" "-Wold-style-cast" "-Woverloaded-virtual")
            if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "4.7" )
              ...
              if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "4.8" )
                list(APPEND CXX_FLAGS  "-Wuseless-cast")
                if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "4.9" )
                  list(APPEND CXX_FLAGS  "-Wconditionally-supported")
                  if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "5.1" )
                    list(APPEND CXX_FLAGS  "-Wformat-signedness" "-Warray-bounds=2" "-Wstrict-null-sentinel" "-Wsuggest-override")
                    if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "6.1" )
                      list(APPEND CXX_FLAGS  "-Wduplicated-cond" "-Wnull-dereference")
                      if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "7.0" )
                        list(APPEND CXX_FLAGS  "-Waligned-new")
                        if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "7.1" )
                          list(APPEND CXX_FLAGS  "-Walloc-zero" "-Walloca" "-Wformat-overflow=2" "-Wduplicated-branches")
                          if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "8.0" )
                            list(APPEND CXX_FLAGS  "-Wcast-align=strict" "-Wformat-truncation=2" "-Wshift-overflow=2" "-Wclass-memaccess")
                            if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "14.0" )
            ...
            if (  JLN_FLAGS_WARNINGS STREQUAL "extensive" )
              if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "8.0" )
                ...
                if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "12.0" )
            ...
            if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "3.9" )
              list(APPEND CXX_FLAGS  "-Wno-undefined-var-template")
              if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "5.0" )
                list(APPEND CXX_FLAGS  "-Wno-inconsistent-missing-destructor-override")
                if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "9.0" )
                  list(APPEND CXX_FLAGS  "-Wno-ctad-maybe-unsupported")
                  if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "10.0" )
                    list(APPEND CXX_FLAGS  "-Wno-c++20-compat")
                    if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "11.0" )
                      list(APPEND CXX_FLAGS  "-Wno-suggest-destructor-override")
                      if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "16.0" )
    ...
    if (   NOT ( JLN_FLAGS_OPTIMIZATION_WARNINGS STREQUAL "default" ) )
      if (  JLN_GCC_CXX_COMPILER )
        if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "9.0" )
          list(APPEND CXX_FLAGS  "-Wpessimizing-move" "-Wredundant-move")
          if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "11.0" )
            list(APPEND CXX_FLAGS  "-Wrange-loop-construct")
            if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "13.0" )
              list(APPEND CXX_FLAGS  "-Wself-move")
              if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "14.0" )
                list(APPEND CXX_FLAGS  "-Wnrvo")
              endif()
            endif()
          endif()
        endif()
      else()
        if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "3.6" )
          list(APPEND CXX_FLAGS  "-Wself-move")
          if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "3.7" )
            list(APPEND CXX_FLAGS  "-Wpessimizing-move" "-Wredundant-move")
            if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "10.0" )
              list(APPEND CXX_FLAGS  "-Wrange-loop-construct")
            endif()
    ...
    if (   NOT ( JLN_FLAGS_UNSAFE_BUFFER_USAGE_WARNINGS STREQUAL "default" ) )
      if (  ( ( JLN_CLANG_LIKE_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "16.0" ) OR ( JLN_CLANG_CL_CXX_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "16.0" ) ) )
        if (  JLN_FLAGS_UNSAFE_BUFFER_USAGE_WARNINGS STREQUAL "off" )
          list(APPEND CXX_FLAGS  "-Wno-unsafe-buffer-usage")
        else()
          list(APPEND CXX_FLAGS  "-Wunsafe-buffer-usage")
        endif()
      endif()
    endif()
    if (   NOT ( JLN_FLAGS_SHADOW_WARNINGS STREQUAL "default" ) )
      if (  JLN_FLAGS_SHADOW_WARNINGS STREQUAL "off" )
        ...
        if (  ( ( JLN_CLANG_LIKE_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "8.0" ) OR ( JLN_CLANG_CL_CXX_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "8.0" ) ) )
        ...
        if (  JLN_FLAGS_SHADOW_WARNINGS STREQUAL "on" )
          ...
          if (  JLN_FLAGS_SHADOW_WARNINGS STREQUAL "all" )
            ...
            if (  ( JLN_GCC_CXX_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "7.1" ) )
        ...
        if (  JLN_FLAGS_SUGGEST_ATTRIBUTES STREQUAL "common" )
          ...
          if (  ( JLN_FLAGS_SUGGEST_ATTRIBUTES STREQUAL "analysis" OR JLN_FLAGS_SUGGEST_ATTRIBUTES STREQUAL "all" ) )
            ...
            if (  JLN_GCC_CXX_COMPILER )
              ...
              if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "5.0" )
                list(APPEND CXX_FLAGS  "-Wno-suggest-final-types" "-Wno-suggest-final-methods")
                if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "5.1" )
    ...
    if (   NOT ( JLN_FLAGS_VAR_INIT STREQUAL "default" ) )
      if (  ( ( JLN_GCC_CXX_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "12.0" ) OR ( JLN_CLANG_LIKE_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "8.0" ) OR ( JLN_CLANG_CL_CXX_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "8.0" ) ) )
        if (  ( ( JLN_CLANG_LIKE_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_LESS_EQUAL "15.0" ) OR ( JLN_CLANG_CL_CXX_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_LESS_EQUAL "15.0" ) ) )
    ...
    if (   NOT ( JLN_FLAGS_WINDOWS_ABI_COMPATIBILITY_WARNINGS STREQUAL "default" ) )
      if (  ( ( JLN_GCC_CXX_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "10.0" ) OR  NOT ( JLN_GCC_CXX_COMPILER ) ) )
        if (  JLN_FLAGS_WINDOWS_ABI_COMPATIBILITY_WARNINGS STREQUAL "on" )
    ...
    if (   NOT ( JLN_FLAGS_REPRODUCIBLE_BUILD_WARNINGS STREQUAL "default" ) )
      if (  ( JLN_GCC_CXX_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "4.9" ) )
      ...
      if (  JLN_FLAGS_WARNINGS_AS_ERROR STREQUAL "on" )
        ...
        if (  JLN_FLAGS_WARNINGS_AS_ERROR STREQUAL "basic" )
          ...
          if (  JLN_GCC_CXX_COMPILER )
            ...
            if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "3.4" )
              list(APPEND CXX_FLAGS  "-Werror=logical-not-parentheses")
              if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "3.6" )
                list(APPEND CXX_FLAGS  "-Werror=delete-incomplete")
                if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "6.0" )
    ...
    if (   NOT ( JLN_FLAGS_CONTROL_FLOW STREQUAL "default" ) )
      if (  JLN_CLANG_EMCC_CXX_COMPILER )
        if (  JLN_FLAGS_CONTROL_FLOW STREQUAL "off" )
          list(APPEND LINK_FLAGS  "-sASSERTIONS=0" "-sSAFE_HEAP=0")
        else()
          list(APPEND LINK_FLAGS  "-sASSERTIONS=1" "-sDEMANGLE_SUPPORT=1")
          if (   NOT ( JLN_FLAGS_SANITIZERS STREQUAL "default" ) AND NOT ( JLN_FLAGS_SANITIZERS STREQUAL "on" OR JLN_FLAGS_SANITIZERS STREQUAL "extra" OR JLN_FLAGS_SANITIZERS STREQUAL "address" ) )
            list(APPEND LINK_FLAGS  "-sSAFE_HEAP=1")
          endif()
        endif()
      else()
        if (  JLN_FLAGS_CONTROL_FLOW STREQUAL "off" )
          if (  ( JLN_GCC_CXX_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "8.0" ) )
            list(APPEND CXX_FLAGS  "-fcf-protection=none")
          else()
            list(APPEND CXX_FLAGS  "-fno-sanitize=cfi" "-fcf-protection=none" "-fno-sanitize-cfi-cross-dso")
            list(APPEND LINK_FLAGS  "-fno-sanitize=cfi")
          endif()
        else()
          if (  ( ( JLN_GCC_CXX_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "8.0" ) OR  NOT ( JLN_GCC_CXX_COMPILER ) ) )
            ...
            if (  ( JLN_FLAGS_CONTROL_FLOW STREQUAL "allow_bugs" AND JLN_CLANG_CXX_COMPILER ) )
              list(APPEND CXX_FLAGS  "-fsanitize=cfi")
              list(APPEND LINK_FLAGS  "-fsanitize=cfi")
              if (   NOT ( JLN_CLANG_CL_CXX_COMPILER ) )
    ...
    if (   NOT ( JLN_FLAGS_COLOR STREQUAL "default" ) )
      if (  ( JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "4.9" OR  NOT ( JLN_GCC_CXX_COMPILER ) ) )
    ...
    if (   NOT ( JLN_FLAGS_LTO STREQUAL "default" ) )
      if (  JLN_FLAGS_LTO STREQUAL "off" )
        ...
        if (  JLN_GCC_CXX_COMPILER )
          ...
          if (  ( ( JLN_FLAGS_LTO STREQUAL "on" OR JLN_FLAGS_LTO STREQUAL "thin_or_nothing" OR JLN_FLAGS_LTO STREQUAL "whole_program" ) AND JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "4.0" ) )
    ...
    if (   NOT ( JLN_FLAGS_STL_HARDENING STREQUAL "default" ) )
      if (   NOT ( JLN_FLAGS_STL_HARDENING STREQUAL "off" ) )
        if (  JLN_FLAGS_STL_HARDENING STREQUAL "fast" )
          if (   NOT ( JLN_CLANG_CL_CXX_COMPILER ) )
            list(APPEND CXX_FLAGS  "-D_GLIBCXX_ASSERTIONS")
          endif()
          list(APPEND CXX_FLAGS  "-D_LIBCPP_HARDENING_MODE=_LIBCPP_HARDENING_MODE_FAST")
        else()
          if (  JLN_FLAGS_STL_HARDENING STREQUAL "extensive" )
            if (   NOT ( JLN_CLANG_CL_CXX_COMPILER ) )
              list(APPEND CXX_FLAGS  "-D_GLIBCXX_ASSERTIONS")
            endif()
            list(APPEND CXX_FLAGS  "-D_LIBCPP_HARDENING_MODE=_LIBCPP_HARDENING_MODE_EXTENSIVE")
          else()
            if (  JLN_FLAGS_STL_HARDENING STREQUAL "debug" )
              if (   NOT ( JLN_CLANG_CL_CXX_COMPILER ) )
                list(APPEND CXX_FLAGS  "-D_GLIBCXX_ASSERTIONS")
              endif()
              list(APPEND CXX_FLAGS  "-D_LIBCPP_HARDENING_MODE=_LIBCPP_HARDENING_MODE_DEBUG")
            else()
              if (   NOT ( JLN_CLANG_CL_CXX_COMPILER ) )
                list(APPEND CXX_FLAGS  "-D_GLIBCXX_DEBUG")
                if (   NOT ( JLN_FLAGS_PEDANTIC STREQUAL "default" ) AND NOT ( JLN_FLAGS_PEDANTIC STREQUAL "off" ) )
                  list(APPEND CXX_FLAGS  "-D_GLIBCXX_DEBUG_PEDANTIC")
                endif()
              endif()
              if (  ( JLN_CLANG_LIKE_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_LESS "18.0" ) )
                if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "8.0" )
                  list(APPEND CXX_FLAGS  "-D_LIBCPP_DEBUG=1")
                endif()
              else()
                list(APPEND CXX_FLAGS  "-D_LIBCPP_HARDENING_MODE=_LIBCPP_HARDENING_MODE_DEBUG" "-D_LIBCPP_ABI_BOUNDED_ITERATORS" "-D_LIBCPP_ABI_BOUNDED_ITERATORS_IN_STRING" "-D_LIBCPP_ABI_BOUNDED_ITERATORS_IN_VECTOR" "-D_LIBCPP_ABI_BOUNDED_UNIQUE_PTR" "-D_LIBCPP_ABI_BOUNDED_ITERATORS_IN_STD_ARRAY")
              endif()
            endif()
          endif()
        endif()
    ...
    if (   NOT ( JLN_FLAGS_SANITIZERS STREQUAL "default" ) )
      if (  ( ( JLN_GCC_CXX_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "4.8" ) OR ( JLN_CLANG_LIKE_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "3.2" ) OR ( JLN_CLANG_CL_CXX_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "4.0" ) ) )
          ...
          if (  JLN_FLAGS_SANITIZERS STREQUAL "thread" )
            ...
            if (  JLN_FLAGS_SANITIZERS STREQUAL "undefined" )
              ...
              if (  JLN_FLAGS_SANITIZERS STREQUAL "undefined_minimal_runtime" )
                list(APPEND CXX_FLAGS  "-fsanitize=undefined")
                list(APPEND LINK_FLAGS  "-fsanitize=undefined")
                if (  ( ( JLN_CLANG_LIKE_COMPILER OR JLN_CLANG_CL_CXX_COMPILER ) AND JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "6.0" ) )
                ...
                if (  JLN_FLAGS_SANITIZERS STREQUAL "scudo_hardened_allocator" )
                  if (  ( JLN_CLANG_CXX_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "13.0" ) )
                    list(APPEND CXX_FLAGS  "-fsanitize=scudo")
                    list(APPEND LINK_FLAGS  "-fsanitize=scudo")
                  endif()
                else()
                  list(APPEND CXX_FLAGS  "-fno-omit-frame-pointer" "-fno-optimize-sibling-calls")
                  if (  ( JLN_FLAGS_SANITIZERS STREQUAL "on" OR JLN_FLAGS_SANITIZERS STREQUAL "extra" OR JLN_FLAGS_SANITIZERS STREQUAL "address" ) )
                    list(APPEND CXX_FLAGS  "-fsanitize=address")
                    list(APPEND LINK_FLAGS  "-fsanitize=address")
                  else()
                    list(APPEND CXX_FLAGS  "-fsanitize=kernel-address")
                    list(APPEND LINK_FLAGS  "-fsanitize=kernel-address")
                  endif()
                  if (  ( JLN_FLAGS_SANITIZERS STREQUAL "on" OR JLN_FLAGS_SANITIZERS STREQUAL "extra" OR JLN_FLAGS_SANITIZERS STREQUAL "kernel" OR JLN_FLAGS_SANITIZERS STREQUAL "kernel_extra" ) )
                    if (  ( ( JLN_GCC_CXX_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "4.9" ) OR JLN_CLANG_LIKE_COMPILER OR JLN_CLANG_CL_CXX_COMPILER ) )
                      list(APPEND CXX_FLAGS  "-fsanitize=undefined")
                      list(APPEND LINK_FLAGS  "-fsanitize=undefined")
                    endif()
                    if (  ( JLN_FLAGS_SANITIZERS STREQUAL "extra" OR JLN_FLAGS_SANITIZERS STREQUAL "kernel_extra" ) )
                      if (  ( ( JLN_GCC_CXX_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "8.0" ) OR ( JLN_CLANG_CXX_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "9.0" ) OR ( JLN_CLANG_CL_CXX_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "9.0" ) ) )
  ...
  if (   NOT ( JLN_FLAGS_CONVERSION_WARNINGS STREQUAL "default" ) )
    if (  ( JLN_GCC_CXX_COMPILER OR JLN_CLANG_LIKE_COMPILER OR JLN_CLANG_CL_CXX_COMPILER OR JLN_ICC_CXX_COMPILER ) )
  ...
  if (  ( JLN_GCC_CXX_COMPILER OR JLN_CLANG_LIKE_COMPILER ) )
    if (   NOT ( JLN_FLAGS_DIAGNOSTICS_SHOW_TEMPLATE STREQUAL "default" ) )
      if (  ( JLN_FLAGS_DIAGNOSTICS_SHOW_TEMPLATE STREQUAL "tree_without_elided_types" OR JLN_FLAGS_DIAGNOSTICS_SHOW_TEMPLATE STREQUAL "tree" ) )
        if (  ( ( JLN_GCC_CXX_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "8.0" ) OR JLN_CLANG_LIKE_COMPILER ) )
          list(APPEND CXX_FLAGS  "-fdiagnostics-show-template-tree")
        endif()
      endif()
      if (  ( JLN_FLAGS_DIAGNOSTICS_SHOW_TEMPLATE STREQUAL "tree_without_elided_types" OR JLN_FLAGS_DIAGNOSTICS_SHOW_TEMPLATE STREQUAL "without_elided_types" ) )
        if (  ( ( JLN_GCC_CXX_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "8.0" ) OR ( JLN_CLANG_LIKE_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "3.4" ) ) )
    ...
    if (   NOT ( JLN_FLAGS_DIAGNOSTICS_FORMAT STREQUAL "default" ) )
      if (  JLN_FLAGS_DIAGNOSTICS_FORMAT STREQUAL "fixits" )
        if (  ( ( JLN_GCC_CXX_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "7.0" ) OR (  NOT ( JLN_GCC_CXX_COMPILER ) AND JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "5.0" ) ) )
          list(APPEND CXX_FLAGS  "-fdiagnostics-parseable-fixits")
        endif()
      else()
        if (  JLN_FLAGS_DIAGNOSTICS_FORMAT STREQUAL "patch" )
          if (  ( JLN_GCC_CXX_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "7.0" ) )
    ...
    if (  JLN_CLANG_EMCC_CXX_COMPILER )
                ...
                if (  JLN_FLAGS_OPTIMIZATION STREQUAL "3" )
                  ...
                  if (  JLN_FLAGS_OPTIMIZATION STREQUAL "size" )
                    ...
                    if (  JLN_FLAGS_OPTIMIZATION STREQUAL "z" )
                      if (  ( JLN_CLANG_CXX_COMPILER OR ( JLN_GCC_CXX_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "12.0" ) ) )
                        list(APPEND CXX_FLAGS  "-Oz")
                      else()
                        list(APPEND CXX_FLAGS  "-Os")
                      endif()
        ...
        if (  JLN_FLAGS_LINKER STREQUAL "mold" )
          ...
          if (  JLN_FLAGS_LINKER STREQUAL "native" )
            ...
            if (  JLN_FLAGS_LINKER STREQUAL "bfd" )
              ...
              if (  ( JLN_FLAGS_LINKER STREQUAL "gold" OR ( JLN_GCC_CXX_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_LESS "9.0" ) ) )
                list(APPEND LINK_FLAGS  "-fuse-ld=gold")
              else()
                list(APPEND LINK_FLAGS  "-fuse-ld=lld")
              endif()
      ...
      if (   NOT ( JLN_FLAGS_STACK_PROTECTOR STREQUAL "default" ) )
        if (  JLN_FLAGS_STACK_PROTECTOR STREQUAL "off" )
          ...
          if (  ( ( JLN_GCC_CXX_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "12.0" ) OR ( JLN_CLANG_CXX_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "14.0" ) ) )
      ...
      if (   NOT ( JLN_FLAGS_RELRO STREQUAL "default" ) )
        if (  JLN_FLAGS_RELRO STREQUAL "off" )
          ...
          if (  JLN_FLAGS_RELRO STREQUAL "on" )
            ...
            list(APPEND LINK_FLAGS  "-Wl,-z,relro,-z,now,-z,noexecstack")
            if (   NOT ( JLN_FLAGS_LINKER STREQUAL "default" ) )
              if (  NOT ( ( JLN_FLAGS_LINKER STREQUAL "gold" OR ( JLN_GCC_CXX_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_LESS "9.0" ) OR ( JLN_FLAGS_LINKER STREQUAL "native" AND JLN_GCC_CXX_COMPILER ) ) ) )
                list(APPEND LINK_FLAGS  "-Wl,-z,separate-code")
              endif()
            endif()
          endif()
        endif()
      ...
      if (   NOT ( JLN_FLAGS_NOEXCEPT_WARNINGS STREQUAL "default" ) )
        if (  ( JLN_GCC_CXX_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "4.6" ) )
      ...
      if (   NOT ( JLN_FLAGS_ANALYZER STREQUAL "default" ) )
        if (  ( JLN_GCC_CXX_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "10.0" ) )
  ...
  if (  ( JLN_MSVC_CXX_COMPILER OR JLN_CLANG_CL_CXX_COMPILER OR JLN_ICL_CXX_COMPILER ) )
    if (   NOT ( JLN_FLAGS_EXCEPTIONS STREQUAL "default" ) )
      if (  JLN_FLAGS_EXCEPTIONS STREQUAL "on" )
    ...
    if (   NOT ( JLN_FLAGS_STL_HARDENING STREQUAL "default" ) )
      if (  JLN_FLAGS_STL_HARDENING STREQUAL "off" )
        list(APPEND CXX_FLAGS  "/D_MSVC_STL_HARDENING=0" "/D_MSVC_STL_DESTRUCTOR_TOMBSTONES=0" "/D_ITERATOR_DEBUG_LEVEL=0" "/D_HAS_ITERATOR_DEBUGGING=0" "/D_SECURE_SCL=0")
      else()
        if (  ( JLN_FLAGS_STL_HARDENING STREQUAL "fast" OR JLN_FLAGS_STL_HARDENING STREQUAL "extensive" ) )
          list(APPEND CXX_FLAGS  "/D_MSVC_STL_HARDENING=1")
        else()
          if (  JLN_FLAGS_STL_HARDENING STREQUAL "debug" )
            list(APPEND CXX_FLAGS  "/D_MSVC_STL_HARDENING=1" "/D_MSVC_STL_DESTRUCTOR_TOMBSTONES=1")
          else()
            list(APPEND CXX_FLAGS  "/D_DEBUG" "/D_MSVC_STL_HARDENING=1" "/D_MSVC_STL_DESTRUCTOR_TOMBSTONES=1")
          endif()
        endif()
      endif()
    endif()
    ...
    if (   NOT ( JLN_ICL_CXX_COMPILER ) )
            ...
            if (  JLN_FLAGS_OPTIMIZATION STREQUAL "2" )
              ...
              if (  ( JLN_FLAGS_OPTIMIZATION STREQUAL "1" OR JLN_FLAGS_OPTIMIZATION STREQUAL "size" ) )
                ...
                if (  JLN_FLAGS_OPTIMIZATION STREQUAL "z" )
                  ...
                  list(APPEND CXX_FLAGS  "/O2")
                  if (  ( ( JLN_MSVC_CXX_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "19.20" ) OR JLN_CLANG_CL_CXX_COMPILER ) )
      ...
      if (   NOT ( JLN_FLAGS_STACK_PROTECTOR STREQUAL "default" ) )
        if (  JLN_FLAGS_STACK_PROTECTOR STREQUAL "off" )
          ...
          if (  JLN_FLAGS_STACK_PROTECTOR STREQUAL "strong" )
            if (  ( JLN_MSVC_CXX_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "19.27" ) )
              list(APPEND CXX_FLAGS  "/guard:ehcont")
              list(APPEND LINK_FLAGS  "/CETCOMPAT")
            endif()
          endif()
          if (   NOT ( JLN_FLAGS_CONTROL_FLOW STREQUAL "default" ) AND NOT ( JLN_FLAGS_CONTROL_FLOW STREQUAL "off" ) )
            list(APPEND CXX_FLAGS  "/guard:cf")
          endif()
        endif()
      endif()
  ...
  if (  JLN_MSVC_CXX_COMPILER )
    if (   NOT ( JLN_FLAGS_ANALYZER STREQUAL "default" ) )
      if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "19.0" )
        if (  JLN_FLAGS_ANALYZER STREQUAL "off" )
          list(APPEND CXX_FLAGS  "/analyze-")
        else()
          list(APPEND CXX_FLAGS  "/analyze")
          if (   NOT ( JLN_FLAGS_ANALYZER STREQUAL "with_external_headers" ) )
            if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "16.10" )
    ...
    if (   NOT ( JLN_FLAGS_MSVC_DIAGNOSTICS_FORMAT STREQUAL "default" ) )
      if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "19.10" )
    ...
    if (  JLN_CXX_COMPILER_VERSION VERSION_LESS "19.13" )
      set(JLN_FLAGS_MSVC_ISYSTEM "default")
    endif()
    if (   NOT ( JLN_FLAGS_MSVC_ISYSTEM STREQUAL "default" ) )
      if (  JLN_FLAGS_MSVC_ISYSTEM STREQUAL "external_as_include_system_flag" )
        if (  JLN_CXX_COMPILER_VERSION VERSION_LESS "19.29" )
          set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} /external:env:INCLUDE /external:W0 /experimental:external " CACHE INTERNAL "")
          set(CMAKE_INCLUDE_SYSTEM_FLAG_CXX "/external:I " CACHE INTERNAL "")
        else()
          set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} /external:env:INCLUDE /external:W0 " CACHE INTERNAL "")
          set(CMAKE_INCLUDE_SYSTEM_FLAG_CXX "/external:I " CACHE INTERNAL "")
        endif()
      else()
        ...
        if (  JLN_FLAGS_WARNINGS STREQUAL "essential" )
          ...
          if (  JLN_FLAGS_WARNINGS STREQUAL "on" )
            ...
            if (  JLN_FLAGS_MSVC_ISYSTEM STREQUAL "default" )
              list(APPEND CXX_FLAGS  "/w14263" "/w14264")
            endif()
            list(APPEND CXX_FLAGS  "/w14265" "/w14296" "/w14444" "/w14555" "/w14557" "/w14608" "/w14905" "/w14906" "/w14917" "/w14928" "/w14545" "/w14546" "/w14547" "/w14548" "/w14549" "/w14822" "/w14692")
            if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "19.0" )
              list(APPEND CXX_FLAGS  "/w14426" "/w14596")
              if (  JLN_FLAGS_MSVC_ISYSTEM STREQUAL "default" )
                list(APPEND CXX_FLAGS  "/w14654")
              endif()
              list(APPEND CXX_FLAGS  "/w15031" "/w15032")
              if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "19.11" )
                list(APPEND CXX_FLAGS  "/w15038")
                if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "19.15" )
                  list(APPEND CXX_FLAGS  "/w14643")
                  if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "19.22" )
                    list(APPEND CXX_FLAGS  "/w14855")
                    if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "19.25" )
                      if (  JLN_FLAGS_MSVC_ISYSTEM STREQUAL "default" )
                        list(APPEND CXX_FLAGS  "/w15204")
                      endif()
                      if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "19.29" )
                        list(APPEND CXX_FLAGS  "/w15233" "/w15240")
                        if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "19.30" )
                          if (  JLN_FLAGS_MSVC_ISYSTEM STREQUAL "default" )
                            list(APPEND CXX_FLAGS  "/w15246")
                          endif()
                          list(APPEND CXX_FLAGS  "/w15249")
                          if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "19.32" )
                            list(APPEND CXX_FLAGS  "/w15258")
                            if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "19.34" )
                              list(APPEND CXX_FLAGS  "/w15263")
                              if (  JLN_FLAGS_MSVC_ISYSTEM STREQUAL "default" )
                                list(APPEND CXX_FLAGS  "/w15262")
                              endif()
                              if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "19.37" )
                                list(APPEND CXX_FLAGS  "/w15267")
                              endif()
                            endif()
                          endif()
                        endif()
            ...
            list(APPEND CXX_FLAGS  "/Wall" "/wd4342" "/wd4350" "/wd4355" "/wd4370" "/wd4371" "/wd4514" "/wd4571" "/wd4577" "/wd4582" "/wd4583" "/wd4587" "/wd4588" "/wd4686" "/wd4710" "/wd4711" "/wd4820" "/wd4866" "/wd4868" "/wd5024" "/wd5025" "/wd5026" "/wd5027" "/wd5243")
            if (  JLN_FLAGS_MSVC_ISYSTEM STREQUAL "default" )
              list(APPEND CXX_FLAGS  "/wd4464" "/wd4548" "/wd4623" "/wd4625" "/wd4626" "/wd4668" "/wd5204")
              if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "19.0" )
                list(APPEND CXX_FLAGS  "/wd4582" "/wd4583")
                if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "19.34" )
            ...
            if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "16.0" )
              ...
              if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "19.39" )
                list(APPEND CXX_FLAGS  "/wd4975")
                if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "19.40" )
                  list(APPEND CXX_FLAGS  "/wd4860" "/wd4861" "/wd5273" "/wd5274")
                  if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "19.41" )
                    list(APPEND CXX_FLAGS  "/wd5306")
                    if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "19.43" )
    ...
    if (   NOT ( JLN_FLAGS_PEDANTIC STREQUAL "default" ) )
      if (   NOT ( JLN_FLAGS_PEDANTIC STREQUAL "off" ) )
        list(APPEND CXX_FLAGS  "/permissive-" "/Zc:inline" "/Zc:referenceBinding" "/Zc:throwingNew")
        if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "19.13" )
          list(APPEND CXX_FLAGS  "/Zc:externConstexpr")
          if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "19.14" )
            list(APPEND CXX_FLAGS  "/Zc:__cplusplus")
            if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "19.25" )
              list(APPEND CXX_FLAGS  "/Zc:preprocessor")
              if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "19.28" )
                list(APPEND CXX_FLAGS  "/Zc:lambda")
                if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "19.34" )
                  list(APPEND CXX_FLAGS  "/Zc:enumTypes")
                  if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "19.35" )
                    list(APPEND CXX_FLAGS  "/Zc:templateScope")
                  endif()
                endif()
              endif()
            endif()
        ...
        if (  JLN_FLAGS_PEDANTIC STREQUAL "as_error" )
          list(APPEND CXX_FLAGS  "/we4608" "/we4928")
          if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "19.31" )
            list(APPEND CXX_FLAGS  "/we5254")
            if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "19.38" )
              list(APPEND CXX_FLAGS  "/we5110")
            endif()
          endif()
        else()
          list(APPEND CXX_FLAGS  "/w14608" "/w14928")
          if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "19.31" )
            list(APPEND CXX_FLAGS  "/w15254")
            if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "19.38" )
        ...
        if (   NOT ( JLN_FLAGS_MSVC_ISYSTEM STREQUAL "default" ) )
          if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "17.0" )
            list(APPEND CXX_FLAGS  "/we4471")
            if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "19.21" )
              list(APPEND CXX_FLAGS  "/we5052")
            endif()
          endif()
        else()
          if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "17.0" )
            list(APPEND CXX_FLAGS  "/w14471")
          endif()
        endif()
      endif()
    endif()
    ...
    if (   NOT ( JLN_FLAGS_SANITIZERS STREQUAL "default" ) )
      if (  JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "19.28" )
        if (  ( JLN_FLAGS_SANITIZERS STREQUAL "on" OR JLN_FLAGS_SANITIZERS STREQUAL "extra" OR JLN_FLAGS_SANITIZERS STREQUAL "address" OR JLN_FLAGS_SANITIZERS STREQUAL "kernel" OR JLN_FLAGS_SANITIZERS STREQUAL "kernel_extra" OR JLN_FLAGS_SANITIZERS STREQUAL "kernel_address" ) )
          if (  ( JLN_FLAGS_SANITIZERS STREQUAL "on" OR JLN_FLAGS_SANITIZERS STREQUAL "extra" OR JLN_FLAGS_SANITIZERS STREQUAL "address" ) )
            list(APPEND CXX_FLAGS  "/fsanitize=address")
            list(APPEND LINK_FLAGS  "/fsanitize=address")
          else()
    ...
    if (  JLN_CLANG_CL_CXX_COMPILER )
      if (   NOT ( JLN_FLAGS_PEDANTIC STREQUAL "default" ) )
        if (   NOT ( JLN_FLAGS_PEDANTIC STREQUAL "off" ) )
          list(APPEND CXX_FLAGS  "/Zc:twoPhase")
          if (  JLN_FLAGS_PEDANTIC STREQUAL "as_error" )
      ...
      if (  JLN_ICL_CXX_COMPILER )
        ...
        if (   NOT ( JLN_FLAGS_CONTROL_FLOW STREQUAL "default" ) )
          if (  JLN_FLAGS_CONTROL_FLOW STREQUAL "off" )
            list(APPEND CXX_FLAGS  "/guard:cf-" "/mconditional-branch=keep")
          else()
            list(APPEND CXX_FLAGS  "/guard:cf")
            if (  JLN_FLAGS_CONTROL_FLOW STREQUAL "branch" )
              list(APPEND CXX_FLAGS  "/mconditional-branch:all-fix" "/Qcf-protection:branch")
            else()
              if (  JLN_FLAGS_CONTROL_FLOW STREQUAL "on" )
                list(APPEND CXX_FLAGS  "/mconditional-branch:all-fix" "/Qcf-protection:full")
              endif()
            endif()
          endif()
        endif()
        ...
        if (  JLN_ICC_CXX_COMPILER )
          ...
          if (   NOT ( JLN_FLAGS_STL_HARDENING STREQUAL "default" ) )
            if (  JLN_FLAGS_STL_HARDENING STREQUAL "debug_with_broken_abi" )
              list(APPEND CXX_FLAGS  "-D_GLIBCXX_DEBUG")
              if (   NOT ( JLN_FLAGS_PEDANTIC STREQUAL "default" ) AND NOT ( JLN_FLAGS_PEDANTIC STREQUAL "off" ) )
                list(APPEND CXX_FLAGS  "-D_GLIBCXX_DEBUG_PEDANTIC")
              endif()
          ...
          if (   NOT ( JLN_FLAGS_STACK_PROTECTOR STREQUAL "default" ) )
            if (  JLN_FLAGS_STACK_PROTECTOR STREQUAL "off" )
              list(APPEND CXX_FLAGS  "-fno-protector-strong" "-U_FORTIFY_SOURCE")
              list(APPEND LINK_FLAGS  "-fno-protector-strong")
            else()
              list(APPEND CXX_FLAGS  "-D_FORTIFY_SOURCE=2")
              if (  JLN_FLAGS_STACK_PROTECTOR STREQUAL "strong" )
                list(APPEND CXX_FLAGS  "-fstack-protector-strong")
          ...
          if (   NOT ( JLN_FLAGS_RELRO STREQUAL "default" ) )
            if (  JLN_FLAGS_RELRO STREQUAL "off" )
              list(APPEND LINK_FLAGS  "-Xlinker-znorelro")
            else()
              if (  JLN_FLAGS_RELRO STREQUAL "on" )
                list(APPEND LINK_FLAGS  "-Xlinker-zrelro")
              else()
          ...
          if (   NOT ( JLN_FLAGS_CONTROL_FLOW STREQUAL "default" ) )
            if (  JLN_FLAGS_CONTROL_FLOW STREQUAL "off" )
              list(APPEND CXX_FLAGS  "-mconditional-branch=keep" "-fcf-protection=none")
            else()
              if (  JLN_FLAGS_CONTROL_FLOW STREQUAL "branch" )
                list(APPEND CXX_FLAGS  "-mconditional-branch=all-fix" "-fcf-protection=branch")
              else()
                if (  JLN_FLAGS_CONTROL_FLOW STREQUAL "on" )
                  list(APPEND CXX_FLAGS  "-mconditional-branch=all-fix" "-fcf-protection=full")
                endif()
              endif()
            endif()
          endif()
