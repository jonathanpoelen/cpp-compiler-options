...
set(_JLN_STL_DEBUG_VALUES default off on extensive debug allow_broken_abi allow_broken_abi_and_bugs)
...
set(JLN_STL_DEBUG "${JLN_STL_DEBUG}" CACHE STRING "Controls the debug level of the STL\n - on: Enable stl assertion or fast hardening mode with libc++\n - extensive: Enable stl assertion or extensive hardening mode with libc++\n - debug: Enable stl assertion or debug hardening mode with libc++\n - allow_broken_abi: Debug mode with ABI incompatibility for more check\n - allow_broken_abi_and_bugs: Like allow_broken_abi, but can make crash with libc++-7 or less")
set_property(CACHE JLN_STL_DEBUG PROPERTY STRINGS "default" "off" "on" "extensive" "debug" "allow_broken_abi" "allow_broken_abi_and_bugs")
if(NOT("${JLN_STL_DEBUG}" STREQUAL ""))
  string(TOLOWER "${JLN_STL_DEBUG}" JLN_STL_DEBUG)
  if(NOT(("default" STREQUAL JLN_STL_DEBUG) OR ("off" STREQUAL JLN_STL_DEBUG) OR ("on" STREQUAL JLN_STL_DEBUG) OR ("extensive" STREQUAL JLN_STL_DEBUG) OR ("debug" STREQUAL JLN_STL_DEBUG) OR ("allow_broken_abi" STREQUAL JLN_STL_DEBUG) OR ("allow_broken_abi_and_bugs" STREQUAL JLN_STL_DEBUG)))
    message(FATAL_ERROR "Unknow value \"${JLN_STL_DEBUG}\" for JLN_STL_DEBUG, expected: default, off, on, extensive, debug, allow_broken_abi, allow_broken_abi_and_bugs")
  endif()
endif()
...
function(jln_init_flags)
  ...
  if("${JLN_VERBOSE_D}" STREQUAL "on" OR "${JLN_VERBOSE_D}" STREQUAL "1")
    ...
    message(STATUS "JLN_STL_DEBUG = ${JLN_STL_DEBUG_D}	[default, off, on, extensive, debug, allow_broken_abi, allow_broken_abi_and_bugs]")
...
function(jln_flags)
  ...
  if (  ( JLN_GCC_CXX_COMPILER OR JLN_CLANG_CXX_COMPILER OR JLN_CLANG_EMCC_CXX_COMPILER ) )
    if (   NOT ( JLN_FLAGS_STL_DEBUG STREQUAL "default" ) )
      if (   NOT ( JLN_FLAGS_STL_DEBUG STREQUAL "off" ) )
        if (  ( JLN_CLANG_CXX_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_LESS "18.0" ) )
          if (  ( JLN_FLAGS_STL_DEBUG STREQUAL "allow_broken_abi" OR JLN_FLAGS_STL_DEBUG STREQUAL "allow_broken_abi_and_bugs" ) )
            if (  ( JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "8.0" OR JLN_FLAGS_STL_DEBUG STREQUAL "allow_broken_abi_and_bugs" ) )
          ...
          if (  JLN_FLAGS_STL_DEBUG STREQUAL "on" )
            list(APPEND CXX_FLAGS  "-D_GLIBCXX_ASSERTIONS" "-D_LIBCPP_HARDENING_MODE=_LIBCPP_HARDENING_MODE_FAST")
          else()
            if (  JLN_FLAGS_STL_DEBUG STREQUAL "extensive" )
              list(APPEND CXX_FLAGS  "-D_GLIBCXX_ASSERTIONS" "-D_LIBCPP_HARDENING_MODE=_LIBCPP_HARDENING_MODE_EXTENSIVE")
            else()
              if (  JLN_FLAGS_STL_DEBUG STREQUAL "debug" )
                list(APPEND CXX_FLAGS  "-D_GLIBCXX_ASSERTIONS" "-D_LIBCPP_HARDENING_MODE=_LIBCPP_HARDENING_MODE_DEBUG")
              else()
                list(APPEND CXX_FLAGS  "-D_GLIBCXX_DEBUG" "-D_LIBCPP_HARDENING_MODE=_LIBCPP_HARDENING_MODE_DEBUG" "-D_LIBCPP_ABI_BOUNDED_ITERATORS" "-D_LIBCPP_ABI_BOUNDED_ITERATORS_IN_STRING" "-D_LIBCPP_ABI_BOUNDED_ITERATORS_IN_VECTOR" "-D_LIBCPP_ABI_BOUNDED_UNIQUE_PTR" "-D_LIBCPP_ABI_BOUNDED_ITERATORS_IN_STD_ARRAY")
              endif()
            endif()
          endif()
        endif()
  ...
  if (  ( JLN_MSVC_CXX_COMPILER OR JLN_CLANG_CL_CXX_COMPILER OR JLN_ICL_CXX_COMPILER ) )
    ...
    if (   NOT ( JLN_FLAGS_STL_DEBUG STREQUAL "default" ) )
      if (  ( ( JLN_MSVC_CXX_COMPILER AND JLN_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "16.7" ) OR JLN_CLANG_CL_CXX_COMPILER ) )
