...
function create_options(options, extra_options)
  if options then
    ...
    return {
      ...
      cxx = options.cxx or (not disable_other_options and _get_extra("jln-cxx")) or nil,
      cxx_version = options.cxx_version or (not disable_other_options and _get_extra("jln-cxx-version")) or nil,
...
function get_flags(options, extra_options)
  ...
  local compiler = options.cxx
  local version = options.cxx_version
  ...
  if ( compiler == "gcc" or compiler == "clang" or compiler == "clang-cl" ) then
        ...
        if compiler == "gcc" then
          ...
          if not ( compversion < 300009 ) then
            ...
            if not ( compversion < 500000 ) then
              ...
              if not ( compversion < 900000 ) then
                insert(jln_cxflags, "-Wno-ctad-maybe-unsupported")
                if not ( compversion < 1000000 ) then
    ...
    if not ( options.windows_abi_compatibility_warnings == "") then
      if ( ( compiler == "gcc" and not ( compversion < 1000000 ) ) or compiler == "clang" or compiler == "clang-cl" ) then
          ...
          if compiler == "gcc" then
            ...
            if ( compiler == "clang" or compiler == "clang-cl" ) then
              ...
              if not ( compversion < 300004 ) then
                insert(jln_cxflags, "-Werror=logical-not-parentheses")
                if not ( compversion < 300006 ) then
  ...
  if ( compiler == "gcc" or compiler == "clang" ) then
    ...
    if not ( options.stl_debug == "") then
      if not ( options.stl_debug == "off" ) then
        ...
        if ( options.stl_debug == "allow_broken_abi" or options.stl_debug == "allow_broken_abi_and_bugs" ) then
          if compiler == "clang" then
            if ( not ( compversion < 800000 ) or options.stl_debug == "allow_broken_abi_and_bugs" ) then
    ...
    if not ( options.elide_type == "") then
      if options.elide_type == "on" then
        if ( compiler == "gcc" and not ( compversion < 800000 ) ) then
          insert(jln_cxflags, "-felide-type")
        end
      else
        if ( ( compiler == "gcc" and not ( compversion < 800000 ) ) or ( compiler == "clang" and not ( compversion < 300004 ) ) ) then
    ...
    if not ( options.diagnostics_show_template_tree == "") then
      if ( ( compiler == "gcc" and not ( compversion < 800000 ) ) or compiler == "clang" ) then
    ...
    if not ( options.noexcept_warnings == "") then
      if ( compiler == "gcc" and not ( compversion < 400009 ) ) then
      ...
      if ( options.msvc_conformance == "all" or options.msvc_conformance == "all_without_throwing_new" ) then
        ...
        if not ( compversion < 1500006 ) then
          ...
          if not ( compversion < 1600005 ) then
            ...
            if not ( compversion < 1600008 ) then
