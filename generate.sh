...
gen compiler | while read comp ; do
  ...
  for g in suggests stl_debug debug sanitizers report_template ; do
    gen compiler $comp $g > "$OUTPUT_DIR"/$comp-$g
  done
  cat "$OUTPUT_DIR"/$comp-stl_debug            "$OUTPUT_DIR"/$comp-debug output/$comp-sanitizers > "$OUTPUT_DIR"/$comp-debug_full
  cat "$OUTPUT_DIR"/$comp-stl_debug_broken_abi "$OUTPUT_DIR"/$comp-debug output/$comp-sanitizers > "$OUTPUT_DIR"/$comp-debug_full_broken_abi
done
