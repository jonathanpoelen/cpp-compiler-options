...
function jln_newoptions(defaults)
  ...
  newoption{trigger="jln-diagnostics-show-template-tree", allowed={{'default'}, {'off'}, {'on'}}, description="Enables printing a tree-like structure showing the common and differing parts of the types"}
  ...
  newoption{trigger="jln-elide-type", allowed={{'default'}, {'off'}, {'on'}}, description="Prints diagnostics showing common parts of template types as \"[...]\""}
  ...
  newoption{trigger="jln-msvc-isystem-with-template-from-non-external", allowed={{'default'}, {'off'}, {'on'}}, description="Warnings concerning template in an external header (requires msvc_isystem)"}
  ...
  newoption{trigger="jln-noexcept-warnings", allowed={{'default'}, {'off'}, {'on'}}, description="Warn when a noexcept-expression evaluates to false because of a call to a function that does not have a non-throwing exception specification (i.e. \"throw()\" or \"noexcept\") but is known by the compiler to never throw an exception."}
  ...
  newoption{trigger="jln-stl-debug", allowed={{'default'}, {'off'}, {'on', 'Enable stl assertion or fast hardening mode with libc++'}, {'extensive', 'Enable stl assertion or extensive hardening mode with libc++'}, {'debug', 'Enable stl assertion or debug hardening mode with libc++'}, {'allow_broken_abi', 'Debug mode with ABI incompatibility for more check'}, {'allow_broken_abi_and_bugs', 'Like allow_broken_abi, but can make crash with libc++-7 or less'}}, description="Controls the debug level of the STL"}
...
function jln_getoptions(values, disable_others, print_compiler)
  ...
  if ( compiler == 'gcc' or is_clang_like ) then
    ...
    if values['diagnostics_show_template_tree'] ~= 'default' then
      if ( ( compiler == 'gcc' and version >= 800000 ) or is_clang_like ) then
        if values['diagnostics_show_template_tree'] == 'on' then
          ...
          table_insert(jln_buildoptions, "-fno-diagnostics-show-template-tree")
        end
      end
    end
    if values['elide_type'] ~= 'default' then
      if values['elide_type'] == 'on' then
        if ( compiler == 'gcc' and version >= 800000 ) then
          table_insert(jln_buildoptions, "-felide-type")
        end
      else
  ...
  if ( compiler == 'gcc' or compiler == 'clang' or compiler == 'clang-emcc' ) then
    if values['stl_debug'] ~= 'default' then
      if values['stl_debug'] ~= 'off' then
        if ( compiler == 'clang' and version < 1800000 ) then
          if ( values['stl_debug'] == 'allow_broken_abi' or values['stl_debug'] == 'allow_broken_abi_and_bugs' ) then
            if ( version >= 800000 or values['stl_debug'] == 'allow_broken_abi_and_bugs' ) then
  ...
  if ( compiler == 'msvc' or compiler == 'clang-cl' or compiler == 'icl' ) then
    ...
    if values['stl_debug'] ~= 'default' then
      if ( ( compiler == 'msvc' and version >= 1600007 ) or compiler == 'clang-cl' ) then
        if values['stl_debug'] == 'off' then
          table_insert(jln_buildoptions, "/D_ITERATOR_DEBUG_LEVEL=0")
        else
          if ( values['stl_debug'] == 'on' or values['stl_debug'] == 'extensive' ) then
            table_insert(jln_buildoptions, "/D_ITERATOR_DEBUG_LEVEL=1")
          else
            table_insert(jln_buildoptions, "/D_ITERATOR_DEBUG_LEVEL=2")
          end
        end
      else
        if values['stl_debug'] == 'off' then
