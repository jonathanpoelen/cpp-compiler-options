...
function jln_newoptions(defaults)
  ...
  newoption{trigger="jln-stl-debug", allowed={{'default'}, {'off'}, {'on'}, {'allow_broken_abi'}, {'allow_broken_abi_and_bugs'}, {'assert_as_exception'}}, description="controls the debug level of the STL"}
...
function jln_getoptions(values, disable_others, print_compiler)
  ...
  if ( compiler == 'gcc' or compiler == 'clang' or compiler == 'clang-cl' or compiler == 'clang-emcc' ) then
    ...
    if values['diagnostics_show_template_tree']~='default' then
      if ( ( compiler == 'gcc' and compversion >= 800000 ) or compiler == 'clang' ) then
    ...
    if values['elide_type']~='default' then
      if values['elide_type'] == 'on' then
        ...
        if ( ( compiler == 'gcc' and compversion >= 800000 ) or ( compiler == 'clang' and compversion >= 300004 ) ) then
    ...
    if values['windows_abi_compatibility_warnings']~='default' then
      if ( ( compiler == 'gcc' and compversion >= 1000000 ) or compiler == 'clang' or compiler == 'clang-cl' or compiler == 'clang-emcc' ) then
    ...
    if values['stl_debug']~='default' then
      if values['stl_debug']~='off' then
        if values['stl_debug'] == 'assert_as_exception' then
          table_insert(jln_buildoptions, "-D_LIBCPP_DEBUG_USE_EXCEPTIONS")
        end
        if ( values['stl_debug'] == 'allow_broken_abi' or values['stl_debug'] == 'allow_broken_abi_and_bugs' ) then
          if compiler == 'clang' then
            if ( compversion >= 800000 or values['stl_debug'] == 'allow_broken_abi_and_bugs' ) then
