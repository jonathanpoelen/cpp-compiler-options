...
local _jln_c_flag_names = {
  ...
  ["jln-bidi-char-warnings"] = true,
  ["bidi_char_warnings"] = true,
  ["jln-color"] = true,
  ["color"] = true,
...
function jln_c_newoptions(defaults)
  ...
  newoption{trigger="jln-bidi-char-warnings", allowed={{'default'}, {'any'}, {'any_and_ucn'}, {'unpaired'}, {'unpaired_and_ucn'}}, description="Enable warnings for possibly misleading Unicode bidirectional control characters. Set to `default` or `unpaired_and_ucn` in cases where some of the source code is expected to include bidirectional control characters."}
  if not _OPTIONS["jln-bidi-char-warnings"] then _OPTIONS["jln-bidi-char-warnings"] = (defaults["bidi_char_warnings"] or defaults["jln-bidi-char-warnings"] or "any") end
...
function jln_c_tovalues(values, disable_others)
  if values then
    ...
    return {
      ...
      ["bidi_char_warnings"] = values["bidi_char_warnings"] or values["jln-bidi-char-warnings"] or (disable_others and "default" or _OPTIONS["jln-bidi-char-warnings"]),
    ...
    return {
      ...
      ["bidi_char_warnings"] = _OPTIONS["jln-bidi-char-warnings"],
...
function jln_c_getoptions(values, disable_others, print_compiler)
  ...
  if ( compiler == 'gcc' or is_clang_like or compiler == 'clang-cl' ) then
    ...
    if values['bidi_char_warnings'] ~= 'default' then
      if ( compiler == 'gcc' and version >= 1200000 ) then
        if values['bidi_char_warnings'] == 'any' then
          table_insert(jln_buildoptions, "-Wbidi-chars=any")
        else
          if values['bidi_char_warnings'] == 'any_and_ucn' then
            table_insert(jln_buildoptions, "-Wbidi-chars=any,ucn")
          else
            if values['bidi_char_warnings'] == 'unpaired' then
              table_insert(jln_buildoptions, "-Wbidi-chars=unpaired")
            else
              table_insert(jln_buildoptions, "-Wbidi-chars=unpaired,ucn")
            end
          end
        end
      end
